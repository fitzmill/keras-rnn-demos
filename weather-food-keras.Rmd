...
numSequences = nrow(food) - sequenceLength

sequences <- array(c(rep.int(0,times=numSequences),rep.int(0,times=sequenceLength),rep.int(0,times=numInputs)),c(numSequences,sequenceLength,numInputs))
for (i in 1:numSequences) {
  sequences[i,,] <- array(as.matrix(food[i:(i+sequenceLength-1),]),c(sequenceLength,numInputs))
}
```
...
feedSequence <- function(model, inputSequence, startState) {
  predictions <- matrix(rep.int(0, nrow(inputSequence) * ncol(startState)), nrow=nrow(inputSequence), ncol=ncol(startState))
